class RewardsController < ApplicationController
  ...
  def create
    ...
    if @reward.save
      redirect_to @reward, notice: "New reward #{@reward.title} created."
    else
  ...
  def update
    if @reward.update valid_params
      redirect_to @reward, notice: "Reward #{@reward.title} updated."
    else
      flash.now[:errors] = @reward.errors.full_messages.join(", ")
      render :edit
    end
  ...
  def destroy
    if @reward.destroy
      redirect_to rewards_path, notice: "Reward #{@reward.title} removed."
    else
      flash.now[:errors] = @reward.errors.full_messages.join(", ")
      redirect_to @reward
    end
